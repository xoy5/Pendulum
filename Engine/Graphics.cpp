...
template<typename T>
inline void Graphics::PutPixel(Vec2_<T> vec, Color c)
{
	PutPixel(int(vec.x), int(vec.y), c);
}
...
{
	return RectI(0, 799, 0, 599);
}

void Graphics::DrawRect(int x_in, int y_in, int size, Color c)
{
	DrawRect(x_in, y_in, size, size, c);
}
void Graphics::DrawRect(RectI rect, Color c)
{
	DrawRect(rect.left, rect.top, rect.GetWidth(), rect.GetHeight(), c);
}
void Graphics::DrawRectByCenter(int x_in, int y_in, int width, int height, Color c)
{
	DrawRect(x_in - int(width / 2), y_in - int(height / 2), width, height, c);
}
void Graphics::DrawRect(int x_in, int y_in, int width, int height, Color c)
{
	for (int y = 0; y < height; y++)
	{
		for (int x = 0; x < width; x++)
		{
			PutPixel(x + x_in, y + y_in, c);
		}
	}
}

void Graphics::DrawLine(Vec2 pos1, Vec2 pos2, Color c, int rope, int width)
{
	...
	Vec2 move = pos2;
	int lineLength = 0;
	while ( rope > lineLength )
	{
		if (GetRect().Contains( Vei2(move) ) )
		{
			PutPixel(move, c);
			for (int i = 0; i < width; i++)
			{
				Vec2 width(i, i);
				if (GetRect().Contains(Vei2(move + width)))
				{
					PutPixel(move + width, c);
				}
				if (GetRect().Contains(Vei2(move - width)))
				{
					PutPixel(move - width, c);
		...
		move += direction;
		lineLength = (move - pos2).GetLength();
	}
}
void Graphics::DrawCircle(int x, int y, int radius, Color c)
{
	...
	for (int y_loop = y - radius + 1; y_loop < y + radius; y_loop++)
	{
		for (int x_loop = x - radius + 1; x_loop < x + radius; x_loop++)
		{
			...
			{
				if (GetRect().Contains(Vei2(x_loop, y_loop)))
				{
...
void Graphics::DrawSprite(int x, int y, const Surface& sprite)
{
	for (int sy = 0; sy < sprite.GetHeight(); sy++)
	{
		...
		{
			PutPixel(sx + x, sy + y, sprite.GetPixel(sx, sy));
		}
	}
}
//////////////////////////////////////////////////
